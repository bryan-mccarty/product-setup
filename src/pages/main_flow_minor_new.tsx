...
function OutcomeChart({ item, overlay = false }) {
  ...
  const baseData = [
    { name: "Thickness", value: Number(item.Thickness) },
    { name: "Sweetness", value: Number(item.sweetness) },
    { name: "Cardboard", value: Number(item.Cardboard) }
  ];

  const goalData = [
    { name: "Thickness", value: GOAL_VALUES.Thickness },
    { name: "Sweetness", value: GOAL_VALUES.sweetness },
    { name: "Cardboard", value: GOAL_VALUES.Cardboard }
  ];
...
export default function OutcomePrioritizationMajor() {
  ...
  const [options, setOptions] = useState([
    ...
    { id: "optionB", name: "Option B", Thickness: 5, sweetness: 6, Cardboard: 5 },
    { id: "optionC", name: "Option C", Thickness: 7, sweetness: 8, Cardboard: 6 }
  ]);
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8">
      ...
      <p className="text-lg font-semibold text-center mt-8">
        Finally, teach Luna the relative importance of your minor outcomes.
      </p>
          ...
          <div className="w-64 bg-white border border-gray-200 rounded-lg shadow-sm">
            ...
            <div className="divide-y divide-gray-200">
              <div className="flex justify-between px-4 py-2">
                ...
                <span className="text-sm">{GOAL_VALUES.Thickness}</span>
              </div>
              <div className="flex justify-between px-4 py-2">
              ...
              <div className="flex justify-between px-4 py-2">
                ...
                <span className="text-sm">{GOAL_VALUES.Cardboard}</span>
              </div>
            </div>
          </div>
        </div>
            ...
            >
              <motion.div layout className="w-64 bg-white border border-gray-200 rounded-lg shadow-sm">
                ...
                <div className="divide-y divide-gray-200">
                  <div className="flex justify-between px-4 py-2">
                    <span className="text-sm font-medium">Thickness</span>
                    <span className="text-sm">{item.Thickness}</span>
                  </div>
                  <div className="flex justify-between px-4 py-2">
                    <span className="text-sm font-medium">Sweetness</span>
                  ...
                  <div className="flex justify-between px-4 py-2">
                    <span className="text-sm font-medium">Cardboard</span>
                    <span className="text-sm">{item.Cardboard}</span>
                  </div>
                </div>
              </motion.div>
            </Reorder.Item>
      ...
      <div className="flex justify-center mt-8">
        <button className="bg-red-600 hover:bg-red-700 text-white font-medium px-6 py-3 rounded-full transition-colors duration-200">
          Confirm & Complete
        </button>
      </div>
