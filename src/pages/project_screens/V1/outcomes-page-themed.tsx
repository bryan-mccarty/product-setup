...
const OutcomeTypeTag = ({ type, small }) => {
  const colors = {
    'Analytical': { bg: 'rgba(96, 165, 250, 0.15)', text: '${theme.accentObjectives}', border: 'rgba(96, 165, 250, 0.3)' },
    'Sensory': { bg: 'rgba(251, 146, 60, 0.15)', text: '#FB923C', border: 'rgba(251, 146, 60, 0.3)' },
    'Consumer': { bg: 'rgba(167, 139, 250, 0.15)', text: '${theme.accentCombinations}', border: 'rgba(167, 139, 250, 0.3)' },
...
const VariableTypeTag = ({ type, small }) => {
  const colors = {
    'Continuous': { bg: 'rgba(96, 165, 250, 0.15)', text: '${theme.accentObjectives}', border: 'rgba(96, 165, 250, 0.3)' },
    'Ordinal': { bg: 'rgba(244, 114, 182, 0.15)', text: '${theme.accentOutcomes}', border: 'rgba(244, 114, 182, 0.3)' },
...
const SourceTag = ({ source }) => {
  ...
  return (
    <span style={{
      ...
      color: isProduct ? '${theme.accentCombinations}' : '#A1A1AA',
...
export default function OutcomesPage() {
  ...
  const {
    outcomes: productOutcomesRaw,           // Product outcomes (brownie mix category)
    outcomeLibrary,                          // Global library
    projectOutcomes: projectOutcomesRaw,     // Project-specific outcomes (user selections)
    projectMetadata,
    setProjectOutcomes,
    addProjectOutcome,
    removeProjectOutcome,
    stepStatuses: contextStepStatuses,
    setStepStatus
  } = useData();
  ...
  const currentStep = 5;
  const steps = [
    { number: 1, name: 'Basic Information', status: 'completed' },
    { number: 2, name: 'Define Goals / Claims', status: 'completed' },
    { number: 3, name: 'Select Inputs', status: 'completed' },
    { number: 4, name: 'Define Constraints', status: 'completed' },
    { number: 5, name: 'Select Outcomes', status: 'current' },
    { number: 6, name: 'Set Objectives', status: 'upcoming' },
    { number: 7, name: 'Prioritize Objectives', status: 'upcoming' },
    { number: 8, name: 'Review', status: 'upcoming' },
  ];
  ...
  const getStepClass = (step: { number: number; status: string }) => {
  ...
  // Product outcomes for Product tab (with component formatting)
  const productOutcomes = useMemo(() => productOutcomesRaw.map(outcome => ({
    ...outcome,
    min: outcome.limits?.split('-')[0] || '0',
    max: outcome.limits?.split('-')[1] || '100'
  })), [productOutcomesRaw]);

  // Library outcomes for Library tab (exclude items already in product or project)
  const libraryOutcomes = useMemo(() => outcomeLibrary.filter(
    libOutcome => !productOutcomesRaw.find(po => po.name === libOutcome.name) &&
                  !projectOutcomesRaw.find(pro => pro.name === libOutcome.name)
  ).map(outcome => ({
    ...outcome,
    min: outcome.limits?.split('-')[0] || '0',
    max: outcome.limits?.split('-')[1] || '100'
  })), [outcomeLibrary, productOutcomesRaw, projectOutcomesRaw]);


  // Project state
  const [projectSummary] = useState(projectMetadata || {
  ...
  // Project outcomes state - local state for UI, synced with context
  const [outcomes, setOutcomes] = useState(() => projectOutcomesRaw.map(outcome => ({
    ...outcome,
    min: outcome.limits?.split('-')[0] || '0',
    max: outcome.limits?.split('-')[1] || '100'
  })));

  // Sync local outcomes to context when they change
  useEffect(() => {
    const projectOutcomesToSave = outcomes.map(outcome => ({
      id: outcome.id,
      name: outcome.name,
      outcomeType: outcome.outcomeType,
      variableType: outcome.variableType,
      description: outcome.description,
      limits: outcome.variableType === 'Continuous' ? `${outcome.minValue || ''}-${outcome.maxValue || ''}` : undefined,
      levels: outcome.levels,
      isDefault: false,
    }));
    setProjectOutcomes(projectOutcomesToSave);
  }, [outcomes, setProjectOutcomes]);
    ...
    <>
      <style>{`
        ...
        * {
        ...
        .btn-primary {
          background: linear-gradient(135deg, ${theme.accentOutcomes} 0%, #EC4899 100%);
          border: none;
          color: #0a0a0f;
          box-shadow: 0 2px 12px rgba(${themeColorRgb}, 0.25);
        }
        ...
        .send-btn {
          width: 36px;
          height: 36px;
          background: linear-gradient(135deg, #8B5CF6 0%, ${theme.accentCombinations} 100%);
          border: none;
          border-radius: 8px;
          color: white;
          cursor: pointer;
      ...
      <div className="page-container">
        <main className="main-content">
          ...
          <div className="form-actions">
            <button className="btn btn-secondary" onClick={() => setStepStatus(5, 'draft')}>
              Save as Draft
            </button>
            <button
