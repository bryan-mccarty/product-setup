...
const InputTypeTag = ({ type, small }) => {
  const colors = {
    'Ingredient': { bg: 'rgba(45, 212, 191, 0.15)', text: '${theme.accentInputs}', border: 'rgba(45, 212, 191, 0.3)' },
...
export default function InputsPage() {
  ...
  const {
    inputs: productInputsRaw,           // Product inputs (brownie mix category)
    inputLibrary,                        // Global library
    projectInputs: projectInputsRaw,     // Project-specific inputs (user selections)
    projectMetadata,
    setProjectInputs,
    addProjectInput,
    removeProjectInput,
  ...
  // Theme color for inputs (cyan)
  const themeColor = theme.accentInputs;

  // Product inputs for Product tab (with component formatting)
  const productInputs = useMemo(() => productInputsRaw.map(input => ({
    ...input,
    suggestedMin: input.levels ? undefined : '0',
    suggestedMax: input.levels ? undefined : '100'
  })), [productInputsRaw]);

  // Library inputs for Library tab (exclude items already in product or project)
  const libraryInputs = useMemo(() => inputLibrary.filter(
    libInput => !productInputsRaw.find(pi => pi.name === libInput.name) &&
                !projectInputsRaw.find(pri => pri.name === libInput.name)
  ).map(input => ({
    ...input,
    suggestedMin: input.levels ? undefined : '0',
    suggestedMax: input.levels ? undefined : '100'
  })), [inputLibrary, productInputsRaw, projectInputsRaw]);
  ...
  // Project inputs state - local state for UI, synced with context
  const [inputs, setInputs] = useState(() => projectInputsRaw.map(input => ({
    ...input,
    // Restore persisted values or set defaults
    minValue: input.minValue || '',
    maxValue: input.maxValue || '',
    levelsText: input.levelsText || (input.levels?.join(', ') || ''),
  ...
  // Sync local inputs to context when they change
  useEffect(() => {
    const projectInputsToSave = inputs.map(input => ({
      id: input.id,
      name: input.name,
      ...
      isDefault: false,
      levels: input.levels,
      // Persist range/levels and status fields
      minValue: input.minValue,
      maxValue: input.maxValue,
      levelsText: input.levelsText,
      status: input.status,
      comment: input.comment,
      source: input.source,
    }));
    setProjectInputs(projectInputsToSave);
  }, [inputs, setProjectInputs]);
  ...
  return (
    <>
      ...
      <ProjectPageLayout
        ...
        chatSubtitle="Input Configuration Assistant"
      ...
      >
          ...
          <div className="form-actions">
            <button className="btn btn-secondary" onClick={() => setStepStatus(3, 'draft')}>
              Save as Draft
            </button>
            <button
      ...
      {/* Chat Panel - Disabled: ProjectPageLayout already provides chat functionality */}
      {false && <aside className="chat-panel">
