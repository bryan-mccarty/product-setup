...
import { useData, Goal, DraftConstraint, DraftObjective } from '../../contexts/DataContext';
...
const CategoryTag = ({ category, theme }: { category: string; theme: any }) => {
  const colors: Record<string, string> = {
    'Ingredient': theme.accentInputs,
    'Processing Condition': theme.accentConstraints,
...
const AutocompleteInput = ({ value, onChange, onSelect, placeholder, library, goalName, theme }: {
  value: string;
  onChange: (val: string) => void;
  ...
  // Use ref to store library for stable reference
  ...
  const suggestions = useMemo(() => {
    ...
    if (!value || value.length < 1) return lib.slice(0, 8);
            ...
            >
              <div className="autocomplete-item-header">
                ...
                <div className="autocomplete-item-tags">
                  <MetricTypeTag type={item.type} theme={theme} />
                  {item.category && <CategoryTag category={item.category} theme={theme} />}
                </div>
              </div>
...
// Constraint Component - Single inline row (defined outside main component to prevent re-creation)
const ConstraintItem = ({ constraint, onUpdate, onDelete, goalName, valueType, theme, calculatedLibrary }: any) => {
...
// Objective Component - Single inline row (defined outside main component to prevent re-creation)
const ObjectiveItem = ({ objective, onUpdate, onDelete, goalName, valueType, theme, calculatedLibrary, outcomeLibrary, onElevateToConstraint }: any) => {
  const operatorOptions = [
  ...
  return (
    <div className="inline-item-row objective-row">
...
// Goal Card Component (defined outside main component to prevent re-creation)
const GoalCard = ({ goal, onUpdate, onDelete, isSelected, onSelect, theme, calculatedLibrary, outcomeLibrary }: any) => {
  const updateGoalCard = (updates: any) => onUpdate({ ...goal, ...updates });

  const toggleCollapse = () => {
    updateGoalCard({ isCollapsed: !goal.isCollapsed });
    if (goal.isCollapsed) {
      onSelect(goal.id);
    }
  };
  ...
  const addItem = (type: string) => {
    ...
    updateGoalCard({ items: [...goal.items, newItem] });
  };

  const elevateToConstraint = (constraintData: any) => {
    ...
    updateGoalCard({ items: [...goal.items, newConstraint] });
  };

  const updateItem = (itemId: string, updates: any) => {
    updateGoalCard({
      items: goal.items.map((item: any) => item.id === itemId ? updates : item)
    });
  };

  const deleteItem = (itemId: string) => {
    updateGoalCard({ items: goal.items.filter((item: any) => item.id !== itemId) });
  };
  ...
  const getGoalSummary = () => {
    ...
    const constraints = goal.items.filter((i: any) => i.type === 'constraint').length;
    const objectives = goal.items.filter((i: any) => i.type === 'objective').length;
    const parts: string[] = [];
    ...
    <div className={`goal-card ${goal.isCollapsed ? 'collapsed' : ''} ${isSelected && !goal.isCollapsed ? 'selected' : ''}`} onClick={handleCardClick}>
      <div className="goal-header">
        ...
        <input
          ...
          value={goal.name}
          onChange={(e) => updateGoalCard({ name: e.target.value })}
          ...
          <div className="value-type-row">
            ...
            <div className="value-type-toggle">
              <button
                ...
                onClick={() => updateGoalCard({ valueType: 'calculated', items: [] })}
              >
              ...
              <button
                ...
                onClick={() => updateGoalCard({ valueType: 'predicted', items: goal.items.filter((i: any) => i.type === 'objective') })}
              >
                <span className="toggle-icon">◎</span>
                Predicted
                <kbd>⌘2</kbd>
              ...
              {goal.items.map((item: any) => (
                item.type === 'constraint' ? (
                  <ConstraintItem
                    ...
                    onUpdate={(updated: any) => updateItem(item.id, updated)}
...
export default function GoalsClaimsPage() {
  ...
  // Create library formats compatible with existing component structure
  const inputLibrary = useMemo(() => inputs.map(input => ({
    id: input.id,
    name: input.name,
    type: 'input',
  ...
  const outcomeLibrary = useMemo(() => outcomes.map(outcome => ({
    id: outcome.id,
    name: outcome.name,
    type: 'outcome',
    category: outcome.outcomeType,
    variableType: outcome.variableType,
    description: outcome.description
  })), [outcomes]);

  const combinationLibrary = useMemo(() => combinations.map(combo => ({
    id: combo.id,
    name: combo.name,
    type: 'combination',
    description: combo.description,
    components: combo.terms.map(t => t.inputName)
  })), [combinations]);

  const calculationLibrary = useMemo(() => calculations.map(calc => ({
    id: calc.id,
    name: calc.name,
    type: 'calculation',
  ...
  const calculatedLibrary = useMemo(() => [
    ...
    ...combinationLibrary,
    ...calculationLibrary
  ], [inputLibrary, combinationLibrary, calculationLibrary]);

  const [projectInfo] = useState(projectMetadata || {
  ...
  const [goals, setGoals] = useState(() => {
    // Load from context if available (for returning to page)
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === '2') {
        ...
        setGoals(prev => {
          ...
          if (!selectedGoal) return prev;
          return prev.map(g => g.id === selectedGoalId
            ? { ...g, valueType: 'predicted', items: g.items.filter((i: any) => i.type === 'objective') }
            : g
          );
        });
        return;
    ...
    <>
      <style>{`
        ...
        .autocomplete-item.highlighted {
          ...
          border-left-color: ${theme.accentInputs};
        }
      ...
      <ProjectPageLayout
        ...
        chatSubtitle="Goals & Claims Assistant"
      ...
      >
        ...
        <div className="goals-list">
          {goals.map(goal => (
            <GoalCard
              ...
              onUpdate={(updated: any) => updateGoal(goal.id, updated)}
        ...
        <div className="form-actions">
          <button className="btn btn-secondary" onClick={() => setStepStatus(2, 'draft')}>
            Save as Draft
          </button>
          <button
            ...
            onClick={() => {
              ...
              setProjectGoals(goals as Goal[]);

              // Derive draft constraints from goals
              const constraints: DraftConstraint[] = [];
              goals.forEach(goal => {
                goal.items
                  .filter((item: any) => item.type === 'constraint')
                  .forEach((item: any) => {
                    const isPrefilled = item.metricRef && (
                      projectInputs.some(i => i.id === item.metricRef?.id) ||
                      projectCombinations.some(c => c.id === item.metricRef?.id)
                    );
                    constraints.push({
                      id: `draft-${item.id}`,
              ...
              // Derive draft objectives from goals
              const objectives: DraftObjective[] = [];
              goals.forEach(goal => {
                goal.items
                  .filter((item: any) => item.type === 'objective')
                  .forEach((item: any) => {
                    const isPrefilled = item.metricRef &&
                      projectOutcomes.some(o => o.id === item.metricRef?.id);
                    objectives.push({
                      id: `draft-${item.id}`,
                      metricName: item.metricName,
                      metricRef: item.metricRef,
