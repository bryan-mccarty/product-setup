...
import { useData, DraftConstraint } from '../../contexts/DataContext';
import { ProjectPageLayout, ChatMessage } from '../../components/project-setup/ProjectPageLayout';

// Unique ID generator
const generateId = () => Math.random().toString(36).substr(2, 9);

// Default constraint tags for product library
const defaultProductTags = [
  { id: 'tag-1', name: 'Regulatory', color: '#EF4444', category: 'constraint' },
  { id: 'tag-2', name: 'Cost Control', color: '#F59E0B', category: 'constraint' },
  { id: 'tag-3', name: 'Quality', color: '#10B981', category: 'constraint' },
  { id: 'tag-4', name: 'Safety', color: '#6366F1', category: 'constraint' },
  { id: 'tag-5', name: 'Nutrition', color: '#EC4899', category: 'constraint' },
];
...
const productConstraints = [
  { 
    ...
    value1: '30', 
    value2: '',
    tags: [defaultProductTags[0], defaultProductTags[4]] // Regulatory, Nutrition
  },
  { 
    id: 'pc-2', 
    targetId: 'combo-4', 
    ...
    value1: '2.50', 
    value2: '',
    tags: [defaultProductTags[1]] // Cost Control
  },
  { 
    id: 'pc-3', 
    targetId: 'input-7', 
    ...
    value1: '325', 
    value2: '375',
    tags: [defaultProductTags[2], defaultProductTags[3]] // Quality, Safety
  },
  { 
    id: 'pc-4', 
    targetId: 'input-5', 
    ...
    value1: '15', 
    value2: '',
    tags: [defaultProductTags[2]] // Quality
  },
  { 
    id: 'pc-5', 
    targetId: 'input-3', 
    ...
    value1: '20', 
    value2: '35',
    tags: [defaultProductTags[1], defaultProductTags[2]] // Cost Control, Quality
  },
  { 
    id: 'pc-6', 
    targetId: 'combo-1', 
...
// Draft constraints now come from context (derived from Goals & Claims page)
...
export default function AllConstraintsPage() {
  ...
  const {
    projectInputs,          // Project-level inputs (user's selections)
    projectCombinations,    // Project-level combinations
    projectConstraints,     // Project-level constraints
    tags: contextTags,
    projectMetadata,
    ...
    stepStatuses,
    setStepStatus,
    draftConstraints: contextDraftConstraints,  // From Goals & Claims page
    setDraftConstraints: setContextDraftConstraints
  } = useData();
  const navigate = useNavigate();

  const productTags = useMemo(() => contextTags.filter(t => t.category === 'constraint'), [contextTags]);
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    {
      id: '1',
      role: 'assistant',
      content: "I'll help you define constraints for your project. Constraints are hard limits that must be satisfied - like maximum sugar content or cost limits. Start by mapping any draft constraints from your goals, or add new ones from your product library."
    }
  ]);
  ...
  const handleSendMessage = (message: string) => {
    setChatMessages([
      ...
      { id: generateId(), role: 'assistant', content: "I'll help with that constraint..." }
    ]);
  };
  ...
  const [constraints, setConstraints] = useState(() => {
    ...
    contextDraftConstraints.forEach(draft => {
  ...
  const [drafts, setDrafts] = useState(() => {
    return contextDraftConstraints.map(draft => {
      const matchedInput = projectInputs.find(i => i.name === draft.metricName);
      const matchedCombo = projectCombinations.find(c => c.name === draft.metricName);
      const matched = matchedInput || matchedCombo;
  ...
  // Sync local constraints to context when they change
  useEffect(() => {
    const constraintsToSave = constraints.map(c => ({
      id: c.id,
      targetId: c.targetId,
    ...
    setProjectConstraints(constraintsToSave);
  }, [constraints, setProjectConstraints]);

  // Sync draft status back to context when drafts are confirmed
  useEffect(() => {
    // Update context with current draft states (confirmed status)
    const updatedDrafts: DraftConstraint[] = drafts.map(d => ({
      id: d.id,
      ...
      goalId: d.goalId,
      goalName: d.goalName || '',
      status: d.mappedConstraintId && !d.needsConfirmation ? 'confirmed' : 'draft',
      isPrefilled: d.isPrefilled || !!d.metricRef,
    }));
    setContextDraftConstraints(updatedDrafts);
  }, [drafts, setContextDraftConstraints]);
  ...
  const themeColor = theme.accentConstraints;
  ...
  return (
    <>
      ...
      <ProjectPageLayout
        ...
        chatSubtitle="Constraint Configuration Assistant"
      ...
      >
                  ...
                  constraints.map((constraint) => {
                    ...
                    return (
                      <div
                        ...
                        style={{
                          borderLeft: needsConfirm ? '3px solid ${theme.accentConstraints}' : undefined,
        ...
        <div className="form-actions">
          <button className="btn btn-large btn-secondary" onClick={() => setStepStatus(4, 'draft')}>
            Save as Draft
          </button>
          <button
                  ...
                  return (
                    <div
                      ...
                      style={{
                        padding: '10px 12px',
                        marginBottom: '6px',
                        background: isCurrentlyMapped ? 'rgba(245, 158, 11, 0.1)' : theme.cardBg,
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        {isCurrentlyMapped && (
                          <span style={{ fontSize: '10px', color: '#FB923C', fontWeight: 600 }}>CURRENT</span>
                        )}
                      </div>
                    </div>
                  );
