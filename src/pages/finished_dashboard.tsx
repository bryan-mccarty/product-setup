...
const CPGCategoryDashboard = () => {
  ...
  const InputTypeTag = ({ type }: { type: string }) => {
    const colors: Record<string, { bg: string; text: string }> = { 'Ingredient': { bg: 'rgba(45, 212, 191, 0.15)', text: '#2DD4BF' }, 'Processing': { bg: 'rgba(251, 146, 60, 0.15)', text: '#FB923C' } };
    const c = colors[type] || { bg: 'rgba(167, 139, 250, 0.15)', text: '#A78BFA' };
  ...
  const OutcomeTypeTag = ({ type }: { type: string }) => {
    const colors: Record<string, { bg: string; text: string }> = { 'Analytical': { bg: 'rgba(96, 165, 250, 0.15)', text: '#60A5FA' }, 'Sensory': { bg: 'rgba(244, 114, 182, 0.15)', text: '#F472B6' }, 'Consumer': { bg: 'rgba(167, 139, 250, 0.15)', text: '#A78BFA' } };
    const c = colors[type] || { bg: 'rgba(113, 113, 122, 0.15)', text: '#A1A1AA' };
  ...
  const VariableTypeTag = ({ type }: { type: string }) => {
    const colors: Record<string, { bg: string; text: string }> = { 'Continuous': { bg: 'rgba(45, 212, 191, 0.12)', text: '#2DD4BF' }, 'Ordinal': { bg: 'rgba(244, 114, 182, 0.12)', text: '#F472B6' }, 'Nominal': { bg: 'rgba(251, 191, 36, 0.12)', text: '#FBBF24' } };
  ...
  const ObjectiveTypeTag = ({ type }: { type: string }) => {
    const config: Record<string, { bg: string; text: string; icon: string }> = { 'maximize': { bg: 'rgba(34, 197, 94, 0.15)', text: '#22C55E', icon: '↑' }, 'minimize': { bg: 'rgba(239, 68, 68, 0.15)', text: '#EF4444', icon: '↓' }, 'target': { bg: 'rgba(96, 165, 250, 0.15)', text: '#60A5FA', icon: '◎' }, 'between': { bg: 'rgba(167, 139, 250, 0.15)', text: '#A78BFA', icon: '↔' } };
  ...
  const ConstraintTypeTag = ({ type }: { type: string }) => {
    ...
    return <span style={{ padding: '2px 6px', borderRadius: '3px', fontSize: '9px', fontWeight: 600, background: 'rgba(251, 146, 60, 0.15)', color: '#FB923C' }}>{labels[type] || type}</span>;
  };

  const ConstraintTagPill = ({ tag }: { tag: string }) => {
    const colors: Record<string, string> = { 'Regulatory': '#EF4444', 'Cost Control': '#22C55E', 'Quality': '#3B82F6', 'Safety': '#F59E0B' };
    const color = colors[tag] || '#71717A';
    return <span style={{ padding: '2px 6px', borderRadius: '3px', fontSize: '9px', fontWeight: 500, background: `${color}20`, color: color, border: `1px solid ${color}40` }}>{tag}</span>;
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: theme.background, fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif", color: theme.text, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ width: '374px', flexShrink: 0, background: 'rgba(0,0,0,0.2)', borderLeft: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column' }}>
          {/* Tabs */}
          <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
            {tabs.map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ flex: 1, padding: '12px 8px', fontSize: '11px', fontWeight: 500, background: activeTab === tab.id ? 'rgba(255,255,255,0.03)' : 'transparent', color: activeTab === tab.id ? tab.color : '#71717A', border: 'none', borderBottom: activeTab === tab.id ? `2px solid ${tab.color}` : '2px solid transparent', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', transition: 'all 0.15s ease' }}>
          ...
          <div style={{ padding: '12px', borderBottom: '1px solid rgba(255,255,255,0.04)', flexShrink: 0 }}>
            <div style={{ position: 'relative', marginBottom: '10px' }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#52525b" strokeWidth="2" style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }}><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg>
              <input type="text" value={panelSearch} onChange={(e) => setPanelSearch(e.target.value)} placeholder={`Search ${activeTab}...`} style={{ width: '100%', padding: '8px 10px 8px 32px', fontSize: '12px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '6px', color: '#E4E4E7', outline: 'none', fontFamily: 'inherit' }} />
            </div>
            <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
              {getFilterOptions().map(filter => (
                <button key={filter} onClick={() => setPanelFilter(filter)} style={{ padding: '4px 8px', fontSize: '10px', fontWeight: 500, background: panelFilter === filter ? `${activeTabData.color}20` : 'rgba(255,255,255,0.03)', color: panelFilter === filter ? activeTabData.color : '#71717A', border: panelFilter === filter ? `1px solid ${activeTabData.color}40` : '1px solid rgba(255,255,255,0.08)', borderRadius: '4px', cursor: 'pointer' }}>{filter}</button>
              ))}
            </div>
          </div>
          ...
          <div className="custom-scrollbar" style={{ flex: 1, overflow: 'auto', padding: '12px' }}>
            {panelItems.length === 0 ? (
              <div style={{ padding: '40px 20px', textAlign: 'center' }}><p style={{ margin: 0, fontSize: '12px', color: '#52525b' }}>No {activeTab} found</p></div>
            ) : (
              panelItems.map((item: any) => {
                if (activeTab === 'inputs') {
                  return (
                    <div key={item.id} style={{ padding: '10px 12px', marginBottom: '6px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)', borderRadius: '8px', display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                      <button onClick={() => toggleInputDefault(item.id)} style={{ padding: '2px', background: 'transparent', border: 'none', cursor: 'pointer', color: item.isDefault ? '#FBBF24' : '#3f3f46', marginTop: '2px' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
                          <span style={{ fontSize: '12px', fontWeight: 600, color: '#E4E4E7' }}>{item.name}</span>
                  ...
                  return (
                    <div key={item.id} style={{ padding: '10px 12px', marginBottom: '6px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)', borderRadius: '8px', display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                      <button onClick={() => toggleOutcomeDefault(item.id)} style={{ padding: '2px', background: 'transparent', border: 'none', cursor: 'pointer', color: item.isDefault ? '#FBBF24' : '#3f3f46', marginTop: '2px' }}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill={item.isDefault ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>
                      </button>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
                  ...
                  return (
                    <div key={item.id} style={{ padding: '10px 12px', marginBottom: '6px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)', borderRadius: '8px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
                        <span style={{ fontSize: '12px', fontWeight: 600, color: '#E4E4E7' }}>{item.targetName}</span>
                        <ObjectiveTypeTag type={item.objectiveType} />
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        {item.objectiveType === 'between' && item.value1 && item.value2 && <span style={{ fontSize: '11px', color: '#A1A1AA', fontFamily: "'JetBrains Mono', monospace" }}>{item.value1} – {item.value2}</span>}
                        {item.objectiveType === 'target' && item.value1 && <span style={{ fontSize: '11px', color: '#A1A1AA', fontFamily: "'JetBrains Mono', monospace" }}>Target: {item.value1}</span>}
                  ...
                  return (
                    <div key={item.id} style={{ padding: '10px 12px', marginBottom: '6px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)', borderRadius: '8px' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <span style={{ fontSize: '11px', color: '#A1A1AA', fontFamily: "'JetBrains Mono', monospace" }}>{item.constraintType === 'between' ? `${item.value1} – ${item.value2}` : item.value1}</span>
        ...
        <div style={{
          ...
          <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <div>
                <h3 style={{ margin: 0, fontSize: '14px', fontWeight: 600, color: '#E4E4E7' }}>Luna AI</h3>
                <p style={{ margin: 0, fontSize: '10px', color: '#71717A' }}>Powered by Claude</p>
              </div>
            </div>
            <button onClick={() => setChatOpen(false)} style={{ background: 'transparent', border: 'none', color: '#71717A', cursor: 'pointer', padding: '4px' }}>
