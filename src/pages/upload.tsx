...
const DataUploadModal = ({ onClose, onSave }) => {
  const { setInputs, setOutcomes, setUploadedData } = useData();
              ...
              <button
                onClick={() => {
                  if (allConfirmed) {
                    ...
                    // Save raw uploaded data to context
                    if (parsedData && columns) {
                      setUploadedData({
                        parsedData: parsedData as Record<string, string | number>[],
                        columns,
                        fileName: (uploadedFile as any)?.name || 'unknown.csv',
                        uploadedAt: Date.now(),
                      });
                    }
