  ...
  const StatusVisualization = ({ current, projectedMean, projectedStd, compact = false }: { current: number; projectedMean: number; projectedStd: number; compact?: boolean }) => {
    ...
    if (current === 0 && projectedMean === 0) {
      return (
        <div style={{ width: `${width}px`, height: `${height}px`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ fontSize: '9px', color: '#52525b', fontStyle: 'italic' }}>Not started</span>
        </div>
      );
    }
    ...
    return (
      <svg width={width} height={height} style={{ overflow: 'visible' }}>
        <rect x={padding.left} y={baseY - 1} width={chartWidth} height={2} fill="rgba(255,255,255,0.08)" rx={1} />
        {[0, 25, 50, 75, 100].map(tick => (
          <line key={tick} x1={padding.left + (tick / 100) * chartWidth} y1={baseY + 2} x2={padding.left + (tick / 100) * chartWidth} y2={baseY + 5} stroke="rgba(255,255,255,0.15)" strokeWidth={1} />
        ))}
        ...
        <circle cx={currentX} cy={baseY} r={4} fill="#E4E4E7" stroke="#0a0a0f" strokeWidth={2} />
        <text x={currentX} y={baseY - 8} textAnchor="middle" fill="#E4E4E7" fontSize="9" fontWeight="600" fontFamily="'JetBrains Mono', monospace">{current}</text>
        <text x={padding.left} y={baseY + 11} textAnchor="start" fill="#52525b" fontSize="8" fontFamily="'Inter', sans-serif">0</text>
        <text x={width - padding.right} y={baseY + 11} textAnchor="end" fill="#52525b" fontSize="8" fontFamily="'Inter', sans-serif">100</text>
      </svg>
    );
  };
    ...
    <div style={{ flex: 1, padding: '20px 24px', minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
          <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 600, color: '#E4E4E7' }}>Projects</h2>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', flexWrap: 'wrap' }}>
          <div style={{ position: 'relative', flex: '0 0 200px' }}>
            ...
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search projects..." style={{ width: '100%', padding: '7px 10px 7px 32px', fontSize: '12px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '6px', color: '#E4E4E7', outline: 'none', fontFamily: 'inherit' }} />
          </div>
          <div style={{ display: 'flex', gap: '4px' }}>
            {['All', 'Starred', 'In Progress', 'High Confidence', 'Not Started'].map((filter) => (
              <button key={filter} onClick={() => setStatusFilter(filter)} style={{ padding: '6px 10px', fontSize: '10px', fontWeight: 500, background: statusFilter === filter ? 'rgba(45, 212, 191, 0.12)' : 'rgba(255,255,255,0.03)', color: statusFilter === filter ? '#2DD4BF' : '#71717A', border: statusFilter === filter ? '1px solid rgba(45, 212, 191, 0.3)' : '1px solid rgba(255,255,255,0.08)', borderRadius: '5px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${theme.border}`, background: theme.cardBgDark }}>
                ...
                <th className="sort-header" onClick={() => handleSort('name')} style={{ padding: '10px 12px', textAlign: 'left', fontSize: '9px', fontWeight: 600, color: '#71717A', textTransform: 'uppercase', letterSpacing: '0.08em', width: '200px', cursor: 'pointer', userSelect: 'none' }}><div style={{ display: 'flex', alignItems: 'center' }}>ID / Name<SortIcon column="name" /></div></th>
                <th className="sort-header" onClick={() => handleSort('rounds')} style={{ padding: '10px 8px', textAlign: 'center', fontSize: '9px', fontWeight: 600, color: '#71717A', textTransform: 'uppercase', letterSpacing: '0.08em', width: '80px', cursor: 'pointer', userSelect: 'none' }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Rounds<SortIcon column="rounds" /></div></th>
                <th className="sort-header" onClick={() => handleSort('owner')} style={{ padding: '10px 8px', textAlign: 'left', fontSize: '9px', fontWeight: 600, color: '#71717A', textTransform: 'uppercase', letterSpacing: '0.08em', width: '120px', cursor: 'pointer', userSelect: 'none' }}><div style={{ display: 'flex', alignItems: 'center' }}>Owner<SortIcon column="owner" /></div></th>
                <th className="sort-header" onClick={() => handleSort('modified')} style={{ padding: '10px 8px', textAlign: 'left', fontSize: '9px', fontWeight: 600, color: '#71717A', textTransform: 'uppercase', letterSpacing: '0.08em', width: '90px', cursor: 'pointer', userSelect: 'none' }}><div style={{ display: 'flex', alignItems: 'center' }}>Modified<SortIcon column="modified" /></div></th>
                <th className="sort-header" onClick={() => handleSort('status')} style={{ padding: '10px 8px', textAlign: 'left', fontSize: '9px', fontWeight: 600, color: '#71717A', textTransform: 'uppercase', letterSpacing: '0.08em', width: '180px', cursor: 'pointer', userSelect: 'none' }}><div style={{ display: 'flex', alignItems: 'center' }}>Status<SortIcon column="status" /></div></th>
                <th style={{ width: '40px', padding: '10px 6px' }}></th>
              </tr>
            </thead>
            <tbody>
              {filteredProjects.length === 0 ? (
                <tr><td colSpan={7} style={{ padding: '40px 20px', textAlign: 'center' }}><p style={{ margin: 0, fontSize: '12px', color: '#71717A' }}>No projects found</p></td></tr>
              ) : (
                filteredProjects.map((project) => (
                  <tr key={project.id} className="table-row" style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', cursor: 'pointer' }}>
                    <td style={{ padding: '10px 6px', textAlign: 'center' }}>
                      <button className="star-btn" onClick={(e) => { e.stopPropagation(); onToggleProjectStar(project.id); }} style={{ padding: '3px', background: 'transparent', border: 'none', cursor: 'pointer', color: project.starred ? '#FBBF24' : '#3f3f46', transition: 'all 0.15s ease' }}>
                    ...
                    <td style={{ padding: '10px 12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <span style={{ fontSize: '13px', fontWeight: 500, color: '#E4E4E7', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{project.name}</span>
                      </div>
                    </td>
                    <td style={{ padding: '10px 8px', textAlign: 'center' }}><span style={{ fontSize: '12px', fontWeight: 600, color: project.roundsCompleted > 0 ? '#E4E4E7' : '#52525b', fontFamily: "'JetBrains Mono', monospace" }}>{project.roundsCompleted}</span></td>
                    <td style={{ padding: '10px 8px' }}><span style={{ fontSize: '12px', color: '#A1A1AA' }}>{project.owner}</span></td>
                    <td style={{ padding: '10px 8px' }}><span style={{ fontSize: '11px', color: '#71717A' }}>{formatDate(project.dateModified)}</span></td>
                    ...
                    <td style={{ padding: '10px 6px', textAlign: 'center', position: 'relative' }}>
                      <button className="menu-btn" onClick={(e) => { e.stopPropagation(); setOpenMenuId(openMenuId === project.id ? null : project.id); }} style={{ padding: '4px', background: openMenuId === project.id ? 'rgba(255,255,255,0.08)' : 'transparent', border: 'none', borderRadius: '4px', cursor: 'pointer', color: '#71717A', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="2" /><circle cx="12" cy="12" r="2" /><circle cx="12" cy="19" r="2" /></svg>
                      </button>
                      {openMenuId === project.id && (
                        <div ref={menuRef} style={{ position: 'absolute', top: '100%', right: '6px', marginTop: '4px', background: '#1a1a22', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '6px', boxShadow: '0 8px 32px rgba(0,0,0,0.5)', zIndex: 100, minWidth: '140px', overflow: 'hidden', animation: 'fadeIn 0.1s ease-out' }}>
                          <button onClick={() => { onDuplicateProject(project.id); setOpenMenuId(null); }} style={{ width: '100%', padding: '8px 12px', fontSize: '12px', background: 'transparent', border: 'none', color: '#E4E4E7', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', textAlign: 'left' }} onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(255,255,255,0.06)'} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
        ...
        {/* Legend */}
        <div style={{ marginTop: '10px', padding: '8px 12px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)', borderRadius: '6px', display: 'flex', alignItems: 'center', gap: '20px' }}>
          <span style={{ fontSize: '9px', color: '#52525b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Legend:</span>
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><div style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#E4E4E7', border: '2px solid #0a0a0f' }} /><span style={{ fontSize: '10px', color: '#71717A' }}>Current desirability</span></div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><div style={{ width: '16px', height: '8px', background: 'linear-gradient(90deg, rgba(96, 165, 250, 0.1), rgba(96, 165, 250, 0.4), rgba(96, 165, 250, 0.1))', borderRadius: '3px' }} /><span style={{ fontSize: '10px', color: '#71717A' }}>Projected range</span></div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><div style={{ width: '10px', height: '0', borderTop: '1px dashed #60A5FA' }} /><span style={{ fontSize: '10px', color: '#71717A' }}>Projected mean</span></div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><div style={{ width: '10px', height: '2px', background: '#22C55E' }} /><span style={{ fontSize: '10px', color: '#71717A' }}>High improvement</span></div>
        </div>
      </div>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
          <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 600, color: '#E4E4E7' }}>Ideas</h2>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
          <div style={{ position: 'relative', flex: '0 0 200px' }}>
            ...
            <input type="text" value={ideasSearchQuery} onChange={(e) => setIdeasSearchQuery(e.target.value)} placeholder="Search ideas..." style={{ width: '100%', padding: '7px 10px 7px 32px', fontSize: '12px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '6px', color: '#E4E4E7', outline: 'none', fontFamily: 'inherit' }} />
          </div>
          <div style={{ flex: 1 }} />
        ...
        <div style={{ background: theme.cardBg, border: `1px solid ${theme.border}`, borderRadius: '10px', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${theme.border}`, background: theme.cardBgDark }}>
                <th style={{ width: '36px', padding: '10px 6px' }}></th>
                <th style={{ padding: '10px 12px', textAlign: 'left', fontSize: '9px', fontWeight: 600, color: '#71717A', textTransform: 'uppercase', letterSpacing: '0.08em', width: '200px' }}>ID / Name</th>
                <th style={{ padding: '10px 8px', textAlign: 'center', fontSize: '9px', fontWeight: 600, color: '#71717A', textTransform: 'uppercase', letterSpacing: '0.08em', width: '80px' }}>Fidelity</th>
                <th style={{ padding: '10px 8px', textAlign: 'left', fontSize: '9px', fontWeight: 600, color: '#71717A', textTransform: 'uppercase', letterSpacing: '0.08em', width: '120px' }}>Source</th>
                <th style={{ padding: '10px 8px', textAlign: 'left', fontSize: '9px', fontWeight: 600, color: '#71717A', textTransform: 'uppercase', letterSpacing: '0.08em', width: '90px' }}>Modified</th>
                <th style={{ padding: '10px 8px', textAlign: 'left', fontSize: '9px', fontWeight: 600, color: '#71717A', textTransform: 'uppercase', letterSpacing: '0.08em', width: '180px' }}>Projection</th>
                <th style={{ width: '40px', padding: '10px 6px' }}></th>
              </tr>
            </thead>
            <tbody>
              {filteredIdeas.length === 0 ? (
                <tr><td colSpan={7} style={{ padding: '40px 20px', textAlign: 'center' }}><p style={{ margin: 0, fontSize: '12px', color: '#71717A' }}>No ideas found</p></td></tr>
              ) : (
                filteredIdeas.map((idea) => (
                  <tr key={idea.id} className="table-row" style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', cursor: 'pointer' }}>
                    <td style={{ padding: '10px 6px', textAlign: 'center' }}>
                      <button className="star-btn" onClick={(e) => { e.stopPropagation(); onToggleIdeaStar(idea.id); }} style={{ padding: '3px', background: 'transparent', border: 'none', cursor: 'pointer', color: idea.starred ? '#FBBF24' : '#3f3f46', transition: 'all 0.15s ease' }}>
                    ...
                    <td style={{ padding: '10px 12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <span style={{ fontSize: '13px', fontWeight: 500, color: '#E4E4E7', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{idea.name}</span>
                      </div>
                    </td>
                    <td style={{ padding: '10px 8px', textAlign: 'center' }}><span style={{ fontSize: '12px', fontWeight: 600, color: '#E4E4E7', fontFamily: "'JetBrains Mono', monospace" }}>{idea.fidelity}%</span></td>
                    ...
                    <td style={{ padding: '10px 8px' }}><span style={{ fontSize: '11px', color: '#71717A' }}>{formatDate(idea.dateModified)}</span></td>
                    ...
                    <td style={{ padding: '10px 6px', textAlign: 'center', position: 'relative' }}>
                      <button className="menu-btn" onClick={(e) => { e.stopPropagation(); setOpenIdeasMenuId(openIdeasMenuId === idea.id ? null : idea.id); }} style={{ padding: '4px', background: openIdeasMenuId === idea.id ? 'rgba(255,255,255,0.08)' : 'transparent', border: 'none', borderRadius: '4px', cursor: 'pointer', color: '#71717A', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="2" /><circle cx="12" cy="12" r="2" /><circle cx="12" cy="19" r="2" /></svg>
                      </button>
                      {openIdeasMenuId === idea.id && (
                        <div ref={ideasMenuRef} style={{ position: 'absolute', top: '100%', right: '6px', marginTop: '4px', background: '#1a1a22', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '6px', boxShadow: '0 8px 32px rgba(0,0,0,0.5)', zIndex: 100, minWidth: '150px', overflow: 'hidden', animation: 'fadeIn 0.1s ease-out' }}>
